...
MSYT_PATH = str(
    util.get_exec_dir() / 'helpers' / 'msyt{}'.format('.exe' if system() == 'Windows' else '')
)
...
def extract_refs(language: str, tmp_dir: Path, files: set = None):
    ...
    if system() == 'Windows':
        ...
        result = subprocess.run(x_args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
    if result.stderr:
...
class TextsMerger(mergers.Merger):
    ...
    def __init__(self, user_only: bool = True):
        super().__init__(
            'game texts', 'Merges changes to game texts',
            'texts.json', options={'user_only': user_only}
        )
    ...
    def generate_diff(self, mod_dir: Path, modded_files: List[Union[str, Path]]):
        ...
        return {
            save_lang: language_diffs[map_lang] for save_lang, map_lang in language_map.items()
        }
    ...
    def get_all_diffs(self):
        return [
            self.get_mod_diff(mod) for mod in util.get_installed_mods() if self.is_mod_logged(mod)
        ]
    ...
    def perform_merge(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            m_args = [
                MSYT_PATH, 'create',
                '-d', str(tmp_dir),
                '-p', 'wiiu' if util.get_settings('wiiu') else 'switch',
                '-o', str(tmp_dir)
            ]
            if system() == 'Windows':
                subprocess.run(
                ...
                subprocess.run(m_args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)

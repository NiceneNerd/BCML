...
def do_403(environ, start_response):
    ...
    return send_simple_text(
        ...
        "Path {} is not allowed.".format(urlpath),
    )
...
def do_404(environ, start_response):
    ...
    return send_simple_text(
        ...
        "Path {} was not found".format(urlpath),
    )
...
def do_405(environ, start_response):
    ...
    return send_simple_text(
        ...
        "Method {} is not allowed on {}".format(environ["REQUEST_METHOD"], urlpath,),
    )
...
def do_options(environ, start_response):
    ...
    """
    start_response("204 No Content", [("Allow", "OPTIONS, GET, HEAD"),])
    return []
...
class StaticContentsApp:
    ...
    def _serve_partial_file(self, environ, start_response, file, filename, mime):
        ...
        response_headers["Content-Range"] = self._compose_content_range(
            start, end, length
        )
        if end is None:
            amount = None
